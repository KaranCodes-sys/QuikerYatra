...
## Typography
Manrope is the preferred product typeface, with a system sans-serif fallback in code so local builds do not depend on remote font fetching. Typography roles are Display, Heading 1, Heading 2, Heading 3, Body, Small and Caption. Decorative Indian fonts are not part of the product UI foundation.
...
## Components
Reusable primitives live in `src/components/ui`. Components should expose small APIs, use tokens, avoid random variants and keep business logic out of rendering. Avoid pretending unfinished components are production-ready; expand primitives as product screens require them.
